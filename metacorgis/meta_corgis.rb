...
class MetaCorgiPerkPackage
  ...
  #   debugger
  #   info_method = ("get_" + name.to_s + "_info").to_sym
  #   happiness_method = ("get_" + name.to_s + "_happiness").to_sym
  #
  #   info = @shopping_list.send(info_method, @package_id)
  #   happiness = @shopping_list.send(happiness_method, @package_id)
  #
  #   text = name.to_s.split('_').map { |w| w.capitalize }.join(' ')
  #
  #   result = "#{text}: #{info}: #{happiness} licks"
  #   happiness > 30 ? "* #{result}" : result
  # end
